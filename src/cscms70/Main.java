...
class VendingMachine {
    ...
    public int getInsertedMoney(){
        return coins[0] + (coins[1]*5) + (coins[2]*10);
    }
...
class CoinProcessor {
    ...
    public boolean returnChange(int money, int[] coins){
        int output[] = {0, 0, 0};
        onebahtCoin = coins[0];
        fivebahtCoin = coins[1];
        tenbahtCoin = coins[2];

        while (money >= 10 && tenbahtCoin >= 1){
            tenbahtCoin--;
            output[2]++;
            money -= 10;
        }
        while (money >= 5 && fivebahtCoin >= 1){
            fivebahtCoin--;
            output[1]++;
            money -= 5;
        }
        while (money >= 1 && onebahtCoin >= 1){
            onebahtCoin--;
            output[0]++;
            money--;
        }
        if (money == 0){
            System.out.println("One-baht Coin: " + output[0]);
            System.out.println("Five-baht Coin: " + output[1]);
            System.out.println("Ten-baht Coin: " + output[2]);
            return true;
        }
        else{
            return false;
